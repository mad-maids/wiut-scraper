...
class Instance {
  ...
  public async collect(page: Page): Promise<void> {
    ...
    const codes: string[] = await page.$$eval("select#ddlclass > option.dropdown-item", (options) => {
      return options.map(option => <string>option.getAttribute("value"));
    });

    const courses: string[] = await page.$$eval("select#ddlclass > option.dropdown-item", (options) => {
